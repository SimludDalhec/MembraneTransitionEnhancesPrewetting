...
def main():
    flist = sys.argv[1]
    XZs = {}
    fWidth = 'prewet_width_Jb1.55_multi_component.txt'
    fits = {}
    fhw = open(fWidth,'w')
    for line in open(flist,'r'):

        fpoly = line.strip().split()[0]
               #ising_data = read_ising(fising)
        fn = re.match('(.*)\.txt',fpoly).group(1)
        matches = re.match('Ti(.*)_Jb(.*)_u(.*)_u2(.*)_M(.*)_idx(.*)_speedup(.*)_pos\.txt',fpoly)
        if matches is not None:
            Ti,Jb,Mu,Mu2,M,idx,speedup = matches.groups(1)
        hPhi = float(Jb)
        p1pos,tether_arr = read_pos_file2(fpoly)
        fn_base = re.match('(.*)_pos\.txt',fpoly).group()

        XZB1,XZB2,fit_params,bulk_conc,area_fraction,densities = get_adsorption(p1pos,tether_arr,fn_base)
        fits[float(Mu)] = fit_params[1]
        if fit_params[0] is not None:
            fhw.write('%0.7f\t%d\t%0.7f\t%0.7f\t%0.7f\t%0.7f\t%0.7f\t%0.7f\t%0.7f\t%0.7f\n' % (round(float(Mu),4), int(idx),bulk_conc,fit_params[0],fit_params[1],fit_params[2],densities[0],densities[1],densities[2],area_fraction))
    fhw.close()
    mus = sorted(fits.keys())
    fit_params = [1/fits[mu] for mu in mus]
    plt.figure()
    plt.plot(mus,fit_params,color='k',linestyle='--')
    plt.scatter(mus,fit_params,100,edgecolor='k',color = 'green')
...
def get_adsorption(poly_data,tethers,fn):
    ...
    if False:
        ...
        surf_dense1 = np.mean(np.mean(lat_3d_shift[:,0:5,:,:,0],axis=0),axis=0)
        surf_dense2 = np.mean(np.mean(lat_3d_shift[:,0:5,:,:,1],axis=0),axis=0)
        surf_denseT = np.mean(np.mean(lat_3d_shift[:,0:5,:,:,2],axis=0),axis=0)
        fig,ax = plt.subplots(3)
        ax[0].imshow(surf_dense1,cmap='Reds')
        ax[1].imshow(surf_dense2,cmap='Blues')
        ax[2].imshow(surf_denseT,cmap='Purples')
        ax[0].set_xticks([])
        ax[0].set_yticks([])
        ax[1].set_xticks([])
        ax[1].set_yticks([])
    ...
    ### 3D Density 
    plt.figure()
    fn_dense_prof = fn + 'density_profile.txt'
    fh = open(fn_dense_prof,'w')
    zstr ='\t'.join(['z'+str(r) for r in range(0,D)])
    fh.write(('%s\t%s\t%s\n') % ('Phase','Species',zstr))
    ...
    denseb1_str = '\t'.join([str(bulk1Densep[z]) for z in range(D)])
    denseb2_str = '\t'.join([str(bulk2Densep[z]) for z in range(D)])
    tetherdense_str = '\t'.join([str(tetherDensep[z]) for z in range(D)])
    diluteb1_str = '\t'.join([str(bulk1Dilutep[z]) for z in range(D)])
    diluteb2_str = '\t'.join([str(bulk2Dilutep[z]) for z in range(D)])
    tetherdilute_str = '\t'.join([str(tetherDilutep[z]) for z in range(D)])
    fh.write('%s\t%s\t%s\n' % ('Dense','Bulk 1',denseb1_str))
    fh.write('%s\t%s\t%s\n' % ('Dense','Bulk 2',denseb2_str))
    fh.write('%s\t%s\t%s\n' % ('Dense','Tether',tetherdense_str))
    fh.write('%s\t%s\t%s\n' % ('Dilute','Bulk 1',diluteb1_str))
    fh.write('%s\t%s\t%s\n' % ('Dilute','Bulk 2',diluteb2_str))
    fh.write('%s\t%s\t%s\n' % ('Dilute','Tether',tetherdilute_str))
    fh.close()
    ...
    plt.plot(bulk1Densep,c='indianred',label = 'Dense Bulk 1')
    plt.plot(bulk2Densep,c='steelblue', label ='Dense Bulk 2')
    plt.plot(tetherDensep,c='firebrick', label ='Dense Tether')

    plt.scatter(np.arange(0,D,1),np.nanmean(lat_3d_dense_av[:,:,0],axis=0),80,edgecolor='k',c='indianred')
    plt.scatter(np.arange(0,D,1),np.nanmean(lat_3d_dense_av[:,:,1],axis=0),80,edgecolor='k',c='steelblue')
    plt.scatter(np.arange(0,5,1),np.nanmean(lat_3d_dense_av[:,0:5,2],axis=0),80,edgecolor='k',c='firebrick')
    params = None
    fit = None
    area_fraction = np.nanmean(area_fraction_arr[:,0])    
    if np.sum(np.isnan(denseBulk)) > 0 or area_fraction < 0.001:
        params,fit = ([None,None,None],0)
        densities = [np.NaN,np.NaN,np.NaN]
        print "NO FIT"
    else:

        params,fit = scipy.optimize.curve_fit(expon, xBulkF, denseBulk, (phi_0, 1, 0),maxfev=1000)
        print "FIT PARAMS:", params, bulk_conc,area_fraction,densities
        plt.plot(xBulk,expon(xBulkF,params[0],params[1],params[2]),c='k')
