...
def main():
    ...
    fising =  fn + '_ising.txt'
    ising_data = read_ising(fising)

    Ti,Jb,Mu,Mu2,M,idx = re.match('Ti(.*)_Jb(.*)_u(.*)_u2(.*)_M(.*)_idx(.*)_speedup.*_pos\.txt',fpoly).groups(1)
    hPhi = float(Jb)
    ...
    get_adsorption(p1pos,tether_arr,ising_data,fn_base,fh_data)
    print(Mu)
    fh_data.close()
    return
...
def read_ising(fn):
    ...
    for line in open(fn):
        ...
        data.append(Lat)
    return np.array(data[1:len(data)-1])
...
def read_pos_file(fn):
    ...
    for line in open(fn,'r'):
        ...
        if len(terms) == 4:
            ...
            if step > step_prev:
                step_prev = step
    return p1pos[1:len(p1pos)-1],tether_arr[1:len(tether_arr)-1]
...
#################
def get_adsorption(poly_data,tethers,ising_data,fn,fh_data):
    av_LAT1,av_LAT2,av_LAT_tether,av_LAT_ising = calc_averaged_lattice(poly_data,tethers,ising_data)
    ...
    for i in range(len(av_LAT1)):
        ...
        for k in range(0,L):
            for j in range(0,L):
                ...
                lat_ising_shift[i,k,j] = av_LAT_ising[i,k,j]
                for z in range(D):
        ...
        for z in range(D):
            ...
            lat_3d_ising_Av[i,0] = np.mean(lat_ising_shift[i,pos_dense[0],pos_dense[1]])
            lat_3d_ising_Av[i,1] = np.mean(lat_ising_shift[i,pos_dilute[0],pos_dilute[1]])
        ...
        if len(pos_dense[0]) > 1:
            ...
            dense_drop_arr[i,3] = lat_3d_ising_Av[i,0]
            dilute_drop_arr[i,3] = lat_3d_ising_Av[i,1]
    ...
    print np.mean(dense_drop_arr,axis=0), np.mean(area_fraction_arr,axis=0)
    return 
...
def calc_averaged_lattice(poly_data,tethers,ising):
    n = min(len(poly_data),len(tethers),len(ising))
    rng = range(n/4-1,n)
    N = (n -n/4+1)/av_int 
    averaged_latticeb1 = np.zeros((N,D,L,L))
    averaged_latticeb2 = np.zeros((N,D,L,L))
    averaged_tether_lattice = np.zeros((N,D,L,L))
    averaged_ising_lattice = np.zeros((N,L,L))
    count_int = 0
    for i in range(len(poly_data)/4,len(poly_data),av_int):
        ...
        for c in range(av_int):
            if count_int < len(averaged_latticeb1) and i+c < len(tethers) and i+c < len(poly_data):
                ...
                #averaged_ising_lattice[count_int,:,:]  += ising[i+c,:,:]/float(av_int)
            count += 1
        count_int += 1
    return averaged_latticeb1,averaged_latticeb2,averaged_tether_lattice,averaged_ising_lattice
