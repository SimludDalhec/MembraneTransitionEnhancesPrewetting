...
out_path = "/home/mnr29/project/PrewettingExperiments/JobsReservoirDynamicsSolid/"
...
def main(float cT1, float Jbulk, float chem_potent1, float chem_potent2,int fidx, double res_speedup):
    ...
    fn = dir_name+"/Jb{0}_u{1}_cT{2}_idx{3}_speedup{4}".format(Jbulk,chem_potent1,cT1,fidx,res_speedup)
    ...
    bulk_state_sys,bulk_state_res = init_state(Nt1)
    for n in range(10):
        tether_state=init_tethers(Nt1)
        tetherLattice=np.zeros((L,L),dtype=np.intc)
        for k in range(Nt1): 
        ...
        sim(bulk_state_sys,bulk_state_res,tether_state,fn,lattice,lattice_res,Nt1,probs,switch_probs,tetherLattice,res_speedup)
...
def init_state(Nt1):
    ...
    return bulk_state_sys,bulk_state_res

def init_tethers(Nt1):
    # Placing tethers 
...
cdef sim(bulk_state_sys,bulk_state_res,tether_state,fn, int[:,:,:,:] lattice,int[:,:,:,:] lattice_res,int Nt1, double[:,:] probs, double[:,:,:] switch_probs, int[:,:] tetherLattice, double res_speedup):
